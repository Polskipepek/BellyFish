...
namespace BellyFish.Source.Game.CheckerBoard {
    class Checkerboard {
        ...
        public bool Exists(char letter, int digit) => letter <= 'h' && digit <= 8 && letter >= 'a' && digit >= 1;
        ...
        public bool CheckIfCheck(PawnColor checkedColor) {
            return GetPawns(checkedColor.Opposite()).SelectMany(p => p.GetAvailableTakes(p.MoveStrategy.GetMoves(this, p))).Any(m => m.NewPawnPos == GetKing(checkedColor).Position);
        }
