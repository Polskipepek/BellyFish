...
namespace BellyFish.Source.Game.Gameplay.Moves.MoveStrategy {
    class QueenMoveStrategy : IMoveStrategy {
        public IEnumerable<Move> GetMoves(Checkerboard checkerboard, Pawn pawn) {
            ...
            var movesLowerRight = checkerboard.FindMovesAlongDirection(pawn, new Position(0, 0) - new Position(1, -1));
            var movesLowerLeft = checkerboard.FindMovesAlongDirection(pawn, new Position(0, 0) - new Position(1, 1));

            var movesUpper = checkerboard.FindMovesAlongDirection(pawn, new Position(0, 1));
            var movesLeft = checkerboard.FindMovesAlongDirection(pawn, new Position(0, 0) - new Position(1, 0));
